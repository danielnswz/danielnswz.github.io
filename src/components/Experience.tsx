  ...
  return (
    <section id="experience" className="section-anchor py-20 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <SectionHeader num="02" title="Experience" subtitle="Where I've built" />
              ...
              <details className="card group open:bg-light-soft open:dark:bg-ink-soft/40 p-5 md:p-6">
                <summary className="flex cursor-pointer flex-col gap-3 md:flex-row md:items-start md:justify-between marker:content-none list-none">
                  <div className="grid gap-1">
                    ...
                    <p className="text-sm text-accent-dark dark:text-accent">{job.company}</p>
                    <p className="text-xs text-ink/50 dark:text-light/50">
                      <span>{job.type}</span> · <span>{job.period}</span> · <span>{job.location}</span>
                    </p>
                  </div>
                  <ChevronDownIcon
                    width={18}
